...
DB_PATH = os.path.join(os.path.dirname(__file__), "timely_digest.db")
...
def get_digest():
    ...
    cursor.execute(
        "SELECT type, COUNT(*) as c FROM timely_items WHERE date(created_at) >= ? GROUP BY type",
        (week_start_str,)
    )
    type_counts = {row["type"]: row["c"] for row in cursor.fetchall()}
    new_total = sum(type_counts.values())

    # 2. Items resolved (completed) this week
    cursor.execute(
        "SELECT COUNT(*) as c FROM timely_items WHERE status = 'completed' AND date(completed_at) >= ?",
        (week_start_str,)
    )
    resolved_total = cursor.fetchone()["c"]

    # 3. Items resolved on time this week
    cursor.execute(
        """SELECT COUNT(*) as c FROM timely_items
    ...
    # 4. Items that became overdue this week (pending + deadline fell within last 7 days)
    cursor.execute(
    ...
    # 5 & 8. Total open items right now, by urgency
    cursor.execute(
        "SELECT urgency, COUNT(*) as c FROM timely_items WHERE status = 'pending' GROUP BY urgency"
    )
    ...
    if most_overdue_row:
        try:
            ...
            days_overdue = 0
        most_overdue = {
            "id": most_overdue_row["id"],
            "title": most_overdue_row["title"],
            "to_whom": most_overdue_row["to_whom"],
            "deadline": most_overdue_row["deadline"],
...
def get_history(limit=50, offset=0):
    """
    Returns all completed items sorted by most recently completed.
    Supports pagination via limit and offset.
    was_overdue and days_taken are computed on read.
    """
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    ...
    for row in rows:
        ...
        # Compute was_overdue
        was_overdue = False
        if item["deadline"] and item["completed_at"]:
            try:
                ...
                was_overdue = completed_date > deadline_date
            except (ValueError, TypeError):
                pass

        # Compute days_taken
        days_taken = 0
        if item["meeting_date"] and item["completed_at"]:
            try:
                completed_date = datetime.datetime.fromisoformat(item["completed_at"]).date()
                meeting_date = datetime.datetime.strptime(item["meeting_date"], "%Y-%m-%d").date()
                days_taken = (completed_date - meeting_date).days
            except (ValueError, TypeError):
                pass
